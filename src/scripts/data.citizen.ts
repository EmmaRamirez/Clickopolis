...
let miner:Citizen = new Citizen('miner', 'miner', '#CCBE93', 0, 'Miners provide +.2 <img src="img/prod.png"> PC and +.2 PS.', { name: 'prod', mod: 'PC', amount: .5 }, { name: 'prod', mod: 'PS', amount: .5 }, {}, {}, true, true, function (amount:number, resources:Collection<Resource>) {
...
let merchant:Citizen = new Citizen('merchant', 'merchant', '#A0D190', 0, 'Merchants sell and trade goods.', { name: 'cash', mod: 'PM', amount: 10 }, {}, {}, {}, true, false, function (amount:number, resources:Collection<Resource>, playerCiv:Civilization) {
  playerCiv.cashPM += amount * this.contrib1.amount;
  u.elt('.cash-PM').textContent = playerCiv.cashPM;
});
