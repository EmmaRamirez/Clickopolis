...
let ruler:Citizen = new Citizen('ruler', 'CEO', 1, 'This is you, their greatest citizen.', {}, {}, {}, {});
ruler.descriptionOverride = true;
let farmer:Citizen = new Citizen('farmer', 'farmer', 0, 'Farmers provide +.2 <img src="img/food.png"> PC and +1.2 PS.', {name: 'food', mod: 'PC', amount: .2 }, { name: 'food', mod: 'PS', amount: 1.2 }, {}, {}, function (resources:Collection<Resource>, amount:number) {
...
let miner:Citizen = new Citizen('miner', 'miner', 0, 'Miners provide +.2 <img src="img/prod.png"> PC and +.2 PS.', { name: 'prod', mod: 'PC', amount: .2 }, { name: 'prod', mod: 'PS', amount: .2 }, {}, {}, function (resources:Collection<Resource>, amount:number) {
...
let woodcutter:Citizen = new Citizen('woodcutter', 'woodcutter', 0, 'Woodcutters provide +.2 <img src="img/prod.png"> PC and +.2 PS', { name: 'prod', mod: 'PC', amount: .2 }, { name: 'prod', mod: 'PS', amount: .2 }, {}, {}, function (resources:Collection<Resource>, amount:number) {
  resources.get('prod').perClick += amount * this.contrib1.amount;
  resources.get('prod').perSecond += amount * this.contrib2.amount;
  u.elt('.r-prod-pc').textContent = resources.get('prod').perClick.toFixed(1) + ' PC';
  u.elt('.r-prod-ps').textContent = resources.get('prod').perSecond.toFixed(1) + ' PS';
